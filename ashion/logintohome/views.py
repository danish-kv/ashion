...
def home(request):
    ...
    men_count = products.objects.filter(is_listed=True, category__name='Men').count()
    kids_count = products.objects.filter(is_listed=True, category__name='Kids').count()
    woman_count = products.objects.filter(is_listed=True, category__name='Women').count()

    hot_trend = products.objects.all().order_by('priority')[:3]
    best_trend = products.objects.all().order_by('selling_price')[:3]
    feature = products.objects.all().order_by('-id')[:3]
